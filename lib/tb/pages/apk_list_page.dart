...
class SvranTbApkListPage extends StatefulWidget {
  ...
  final FutureOr<void>? Function()? fa;

  const SvranTbApkListPage({Key? key, required this.propertiesList, required this.tbFileListIndex, this.fa})
      : super(key: key);
...
class _SvranTbApkListPageState extends State<SvranTbApkListPage> {
      ...
      child: GestureDetector(
        onSecondaryTap: () {
          showMenu(
            ...
            items: [
              PopupMenuItem(
                child: const Text('删除(仅保留信息文件)'),
                onTap: () => deleteAllExcludeInfo(item, canDelAPk, propertiesList, index, tbFileListIndex),
              ),
              PopupMenuItem(
        ...
        child: MouseRegion(
          ...
          child: ListTile(
            ...
            trailing: IconButton(
              ...
              onPressed: () {
                deleteDialog('此删除会删除数据和无其他备份引用的Apk,以及备份信息文件。\n\n如果想只删除数据或者只删除Apk,请使用子菜单的删除功能').then((dv) {
                  if (dv == true) {
  ...
  void deleteAll(PropertiesModel item, bool canDelAPk, List<PropertiesModel> propertiesList, index, tbFileListIndex) {
    Future.delayed(
      const Duration(seconds: 0),
      () => deleteDialog('删除数据(和无其他备份引用的Apk)').then((dv) {
        if (dv == true) {
          if (item.dataFile.existsSync()) item.dataFile.deleteSync();
          if (canDelAPk && item.apkFile.existsSync()) item.apkFile.deleteSync();
          if (!canDelAPk) svranToast("Apk备份有被其他引用,跳过删除");
          if (item.propertiesFile.existsSync()) item.propertiesFile.deleteSync();
          propertiesList.removeAt(index);
          if (propertiesList.isEmpty) {
            svranToast("已删除完");
  ...
  void deleteAllExcludeInfo(
      PropertiesModel item, bool canDelAPk, List<PropertiesModel> propertiesList, index, tbFileListIndex) {
    Future.delayed(
      const Duration(seconds: 0),
      () => deleteDialog('删除数据(和无其他备份引用的Apk), 但保留备份信息描述文件').then((dv) {
        if (dv == true) {
          if (item.dataFile.existsSync()) item.dataFile.deleteSync();
          if (canDelAPk && item.apkFile.existsSync()) item.apkFile.deleteSync();
          if (!canDelAPk) svranToast("Apk备份有被其他引用,跳过删除");
